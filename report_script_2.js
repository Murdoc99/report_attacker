...
var scriptConfig = {
    scriptData: {
        prefix: 'setGetVillageNotes_spectatorAttacker',
        name: 'Set/Get Village Notes (Spectator → Attacker)',
        version: 'v1',
        author: 'Murdoc',
    },
    translations: {
        en_DK: {
            'Set/Get Village Notes':'Set/Get Village Notes (Spectator → Attacker)',
            Help: 'Help',
...
// Names (trimmed)
const attackerPlayerName = jQuery('table#attack_info_att')[0].rows[0].cells[1].textContent.trim();
const defenderPlayerName = jQuery('table#attack_info_def')[0].rows[0].cells[1].textContent.trim();

if (defenderPlayerName !== '---') {

    // Village IDs
    const attackerVillageId = jQuery('table#attack_info_att')[0]
        .rows[1].cells[1].getElementsByTagName('span')[0]
        .getAttribute('data-id');

    const defenderVillageId = jQuery('table#attack_info_def')[0]
        .rows[1].cells[1].getElementsByTagName('span')[0]
        .getAttribute('data-id');

    const isMeAttacker = attackerPlayerName === game_data.player.name;
    const isMeDefender = defenderPlayerName === game_data.player.name;

    // Decide where to save note
    if (isMeDefender) {
        villageId = attackerVillageId;      // you got attacked -> tag attacker
    } else if (isMeAttacker) {
        villageId = defenderVillageId;      // you attacked -> tag defender
    } else {
        villageId = attackerVillageId;      // spectator -> attacker (your variant)
    }

    // (Optional but recommended) restore troop reading so Off/Def numbers work
    jQuery('#attack_info_att_units tr:eq(1) td.unit-item').each(function () {
        attackingVillageTroops.push(parseInt(jQuery(this).text().trim(), 10));
    });

    // ... your existing code continues here (game_data.units.map etc.)
